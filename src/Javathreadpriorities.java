...
The thread scheduler is responsible for assigning the processor to a thread. But the question is on what basis the
thread scheduler decides that a particular thread will run before other threads. Here comes the concept of priority in the picture.

1.Every single thread created in Java has some priority associated with it.
2.The programmer can explicitly assign the priority to the thread. Otherwise, JVM automatically assigns
 priority while creating the thread.
3.In Java, we can specify the priority of each thread relative to other threads. Those threads having higher priorities
 get greater access to the available resources than lower priorities threads.
4.Thread scheduler will use priorities while allocating processor.
5.The valid range of thread priorities is 1 to 10 (but not 0 to 10), where 1 is the least priority, and
 10 is the higher priority.
...
NORM_PRIORITY: This is the default priority automatically assigned by JVM to a thread if a programmer does not explicitly
set the priority of that thread. Value is 5.
...
This method is used to set the priority of a thread. IllegalArgumentException is thrown if the priority given
by the user is out of the range [1,10].
